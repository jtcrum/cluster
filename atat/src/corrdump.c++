...
/*
    //output the cluster pairs, added by Tong
    cout << "clusters in unit cell:"
    cout << "cluster multiplicity: " << pmulticlus->get_size() << endl;
    cout << "site number: " << (*pmulticlus)[0].clus.get_size() << endl;
    for (int i=0; i<(pmulticlus->get_size()); i++){
      for (int j=0; j<((*pmulticlus)[i].clus.get_size()); j++){
        cout << (*pmulticlus)[i].clus[j] << endl;
      }
    }
    cout << endl;//finish
*/
